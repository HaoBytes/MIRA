...
class TimeAwareWindowDataset(Dataset):
    def __init__(self, dataset: TimeAwareDataset, context_length, prediction_length=0, normalize=False):
        ...
        # Precompute all valid window starting points (in raw index space)
        self.valid_windows = []
    ...
    def __getitem__(self, idx):
        ...
        # Slice the full window from sequence, time, and mask
        full_sequence = item['sequence'][start_idx: start_idx + self.window_length]
        full_time = item['time'][start_idx: start_idx + self.window_length]
        full_mask = item['mask'][start_idx: start_idx + self.window_length]

        # Split into context and prediction segments
        ...
        # Extract valid observations (mask == 1)
        inputs = context_seq[context_mask == 1]
        input_time = context_time[context_mask == 1]

        labels = pred_seq[pred_mask == 1]
        label_time = pred_time[pred_mask == 1]

        # Generate loss mask for valid prediction values (all 1s after filtering)
        loss_masks = np.ones_like(labels, dtype=np.int32)

        # Optional normalization (on valid input values)
        if self.normalize and len(inputs) > 0:
            mean = inputs.mean()
            std = inputs.std()
        ...
        time_deltas = np.concatenate([
            input_time[1:] - input_time[:-1],
            label_time - input_time[-1:]
        ])

...
class OptimizedTimeSeriesInputEmbedding(nn.Module):
    ...
    def __init__(self, config: TimeMoeConfig):
        ...
        self.time_variance_threshold = nn.Parameter(torch.tensor(0.1))
    ...
    def is_regular_timing(self, time_values: torch.Tensor) -> torch.Tensor:
        ...
        deltas = time_values[:, 1:] - time_values[:, :-1]
        variances = torch.var(deltas, dim=1, keepdim=True)
        is_regular = variances < (F.softplus(self.time_variance_threshold) + self.epsilon)
        # [B, 1] -> [B, 1, 1]
        return is_regular.unsqueeze(-1)  # Shape: [B, 1, 1]

    def forward(self, x: torch.Tensor, time_values: torch.Tensor = None) -> torch.Tensor:
        # Value processing
        ...
        is_regular = self.is_regular_timing(time_values)  # [B, 1, 1]
        time_emb = torch.zeros_like(value_emb)


        if not torch.all(is_regular):
            scaled_time = time_values.unsqueeze(-1)
            time_emb = self.time_emb(scaled_time)

        gate_input = torch.cat([value_emb, time_emb * (~is_regular)], dim=-1)

        gate = 0.5 + 0.5 * self.fusion_gate(gate_input)  # ∈ [0.5, 1.0]
        fused_emb = gate * value_emb + (1 - gate) * time_emb

        # Final fusion and normalization
        return self.norm(fused_emb * (~is_regular) + value_emb * is_regular)


class TimeAwareRotaryEmbedding(nn.Module):
    """Rotary Position Embedding with temporal awareness"""
    def __init__(self, dim, max_t=1e5, base=10000):
        super().__init__()
        self.dim = dim
        self.base = base
        self.max_t = max_t
    ...
    def _compute_freqs(self, time_values):
        """Compute dynamic frequencies based on time values"""
        scaled_time = time_values * self.time_scale  # [batch*num_heads, seq_len, 1]
        scaled_time = scaled_time.squeeze(-1)       #  [batch*num_heads, seq_len]
        freqs = torch.einsum('b s, d -> b s d', scaled_time, self.inv_freq)
        return torch.cat([freqs, freqs], dim=-1)

    def forward(self, q: torch.Tensor, k: torch.Tensor, time_values: torch.Tensor):
        """Apply temporal-aware rotation""" 
        batch_size, num_heads, seq_len, head_dim = q.size() # Get original q shape: [batch_size, num_heads, seq_len, head_dim]

        q_merged = q.reshape(batch_size * num_heads, seq_len, head_dim)
        k_merged = k.reshape(batch_size * num_heads, seq_len, head_dim)

        # Handle time_values input: original shape [batch_size, seq_len, 1]
        # Expand to [batch_size * num_heads, seq_len, 1]
        if time_values.dim() == 3:
            time_values = time_values.unsqueeze(1)  # [batch, 1, seq_len, 1]
            time_values = time_values.expand(-1, num_heads, -1, -1)  # [batch, num_heads, seq_len, 1]
            time_values = time_values.reshape(batch_size * num_heads, seq_len, 1)
            
        
        # Compute dynamic frequencies
        freqs = self._compute_freqs(time_values) # [batch*num_heads, seq_len, dim]
        
         # Apply rotation to queries and keys
        cos = torch.cos(freqs)
        sin = torch.sin(freqs)
        q_rot = q_merged * cos + rotate_half(q_merged) * sin
        k_rot = k_merged * cos + rotate_half(k_merged) * sin
        
        # Restore original shape
        q_rot = q_rot.view(batch_size, num_heads, seq_len, head_dim)
        k_rot = k_rot.view(batch_size, num_heads, seq_len, head_dim)
...
class TimeMoeAttention(nn.Module):
    ...
    def __init__(self, config: TimeMoeConfig, layer_idx: Optional[int] = None):
        ...
        self.rotary_emb = (
            TimeAwareRotaryEmbedding(self.head_dim) 
            if config.time_aware_rotary 
            else TimeMoeRotaryEmbedding(
                self.head_dim,
    ...
    def forward(
        ...
        if isinstance(self.rotary_emb, TimeAwareRotaryEmbedding):
            query_states, key_states, cos, sin = self.rotary_emb(query_states, key_states, time_values)
        else:
...
class TimeMoeModel(TimeMoePreTrainedModel):
    ...
    def __init__(self, config: TimeMoeConfig):
        ...
        self.embed_layer = (
            OptimizedTimeSeriesInputEmbedding(config) 
            if config.time_aware 
            else TimeMoeInputEmbedding(config)
        )

        if config.time_aware:
            self.temporal_encoder = nn.LSTM(
                input_size=config.temporal_embed_dim,
    ...
    def forward(
            ...
            time_values: torch.FloatTensor = None,
            attention_mask: Optional[torch.Tensor] = None,
            position_ids: Optional[torch.LongTensor] = None,

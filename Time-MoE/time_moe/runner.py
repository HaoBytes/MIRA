...
from time_moe.datasets.timeawared_dataset import TimeAwareJSONLDataset, TimeAwareEvalDataset
...
class TimeMoeRunner:
    def __init__(
            ...
            seed: int = 9899,
            custom_config: TimeMoeConfig = None
    ):
    ...
    def load_model(self, model_path: str = None, from_scratch: bool = False, **kwargs):
        ...
        if from_scratch:
            config_loader = lambda: (
                self.custom_config if self.custom_config 
                else TimeMoeConfig.from_pretrained(model_path, _attn_implementation=attn)
            )
            
            config = config_loader()          
            log_in_local_rank_0(f'using {"CUSTOM" if self.custom_config else "DEFAULT"} config')
            model = TimeMoeForPrediction(config)
        else:
            model = TimeMoeForPrediction.from_pretrained(model_path, **kwargs)
        ...
        if local_rank == 0:
            ...
            activated_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
            print(f"total parameters: {total_params/1e9:.1f}B | active parameters:{activated_params/1e9:.1f}B")
    ...
    def get_train_dataset(self, data_path, max_length, normalization_method, time_aware=train_config.get('time_aware', False)):
        log_in_local_rank_0('Loading dataset...')
        if time_aware:
            dataset = TimeAwareJSONLDataset(data_path, auto_quantize=True)
            log_in_local_rank_0('Processing time awared dataset to fixed-size sub-sequences...')
            window_dataset = TimeAwareWindowDataset(dataset, context_length=max_length, prediction_length=0)
        else:

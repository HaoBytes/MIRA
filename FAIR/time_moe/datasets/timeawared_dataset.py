...
class TimeAwareJSONLDataset(TimeSeriesDataset):
    ...
    #             else:
    #                 # 日志记录移到 get_sequence_length_by_idx 的 except 块中可能更好
    #                 return 0 # 返回 0 而不是 None 或错误
    ...
    #          # 在 _by_idx 中记录更合适
    #          return 0
    #     except Exception: # 捕获其他可能的错误
    #          # 在 _by_idx 中记录更合适
    #          return 0
    
    def get_sequence_length_by_idx(self, seq_idx):
        """
        实现基类要求的获取长度的方法，并包含详细的错误处理。
        """
        try:
            length = self.get_sequence_length(seq_idx)
            # 额外检查 get_sequence_length 是否返回了非预期类型 (理论上不应发生)
            if not isinstance(length, int) or length < 0:
                 ...
                 return 0
            return length
        except Exception as e: # 捕获 get_sequence_length 可能漏掉的异常
    ...
    def get_sequence_length(self, seq_idx):
        try:
            ...
            if isinstance(item, dict):
                sequence_data = item.get('sequence') 
                if sequence_data is not None and hasattr(sequence_data, '__len__'):
                    return len(sequence_data) # <--- 这里应该能正确处理含 None 的列表
                else: # sequence 键不存在，或者是 None，或者不是列表/数组
                    logger.warning(f"Item at seq_idx {seq_idx} is dict but 'sequence' key is problematic. Returning length 0.", once=True)
                    return 0
            elif item is not None and hasattr(item, '__len__'): 
                return len(item)
            else: # item 是 None 或其他不支持 len 的类型

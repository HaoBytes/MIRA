...
class TimeAwareJSONLDataset(TimeAwareDataset):
    ...
    def _fit_normalizer_and_infer_quantization(self, time_normalization, auto_quantize, sample_size):
        ...
        if time_normalization == 'standard':
            ...
            try:
                ...
                logger.info(f"Fitted StandardScaler for time: mean={self.time_normalizer.mean_[0]:.4f}, scale={self.time_normalizer.scale_[0]:.4f}")
            except ValueError as e:
                 logger.error(f"Error fitting StandardScaler: {e}. Disabling time normalization.")
                 self.time_normalizer = None
        elif time_normalization is not None:
            logger.warning(f"Unsupported time_normalization method: {time_normalization}. Normalization disabled.")
    ...
    def get_sequence_length(self, seq_idx):
        # Get length from underlying data without loading full item if possible
        # This might be slow if data is purely jsonl lines
        item = self.data[seq_idx]
        if isinstance(item, dict):

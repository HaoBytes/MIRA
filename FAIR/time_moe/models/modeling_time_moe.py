...
TIME_MOE_ATTENTION_CLASSES = {
    ...
    'flash_attention_2': TimeMoeAttention,
}
...
class TerminalODEBlock(nn.Module):
    ...
    def forward(self, h_N: torch.Tensor, t_N: torch.Tensor, t_Nplus1: torch.Tensor):
        ...
        if torch.any(delta_t <= 0):
             ...
             # Create time evaluation points relative to t_N (i.e., 0 to delta_t)
             # We need a t_eval tensor for each item if delta_t varies. This is tricky with odeint.
             # Simplification: Solve for max delta_t and interpolate? Or solve per item?
             # Let's try solving for max interval and hope dynamics handle smaller intervals ok.
             # Or, simplest: Assume delta_t is roughly the same or positive, solve with max interval.
             # A more robust solution would iterate or use a batch-compatible solver if intervals vary significantly.

             # Using max delta_t for simplicity (may not be ideal if delta_t varies widely)
             ...
             # How to get state at individual delta_t_valid if solver only gives state at max_delta_t?
             # Re-solve for each required delta_t, or use interpolation if supported.
             # `odeint` can accept a list of time points.
             # Let's evaluate at [0, delta_t_i] for each item i. This requires a loop or batched solver.

             # --- Reverting to simpler logic: Assume delta_t > 0 for all intended items ---
             # --- Or filter and solve only for valid ones, returning h_N for invalid ---
             if torch.any(delta_t <= 0):
        ...
        # We integrate from relative time 0 to delta_t
        solution = odeint(
            self.ode_func,
...
class TimeMoeForPrediction(TimeMoePreTrainedModel, TSGenerationMixin):
    ...
    def forward(
        ...
        # We need the hidden state corresponding to the *last input token*
        # to extrapolate from.
        # If using KV cache, the input sequence length (q_len) might be 1.
        # hidden_states_all will have shape [B, q_len, D].
        # We always take the state from the very end of this sequence.
        ...
        if self.use_terminal_ode and self.ode_extrapolation_block is not None:
            if next_target_time_values is None:
                 ...
                 # Or we need next_target_time_values even in training if loss depends on it.
                 # For now, assume ODE is primarily for inference or specific training setups.
                 if self.training:

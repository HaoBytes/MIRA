...
class TimeMoeConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        #      kwargs["_attn_implementation"] = "eager" # Or "flash_attention_2" if preferred/available
        # self._attn_implementation = kwargs.pop('_attn_implementation', 'eager')

        # # Validation for MoE
        # if not self.use_dense and self.num_experts <= 0:
        #      logger.warning("MoE enabled (use_dense=False) but num_experts <= 0. Defaulting num_experts to 1.")
        #      self.num_experts = 1
        # if self.use_dense and self.apply_aux_loss:
        #     logger.warning("`use_dense` is True, disabling `apply_aux_loss`.")

...
from time_moe.datasets.time_aware_dataset import JSONLTimeAwareDataset, MaskedTimeAwareWindowDataset, masked_time_aware_collate_fn
...
class TimeMoeRunner:
    ...
    def train_model(self, from_scratch: bool = False, **kwargs):
        ...
        # --- Load Dataset ---
        is_time_aware_dataset = train_config.get('time_aware', True) # Default to True if using jsonl
        data_path = train_config['data_path']
        max_length = train_config['max_length'] # This is context_length

        current_data_collator = None # Default for non-time-aware

        if use_time_aware_dataset_flag and data_path.endswith('.jsonl'):
            log_in_local_rank_0('Loading Time-Aware JSONL Dataset...')
            time_norm_method = train_config.get('time_normalization_method', 'standard')
            quantize_res = train_config.get('time_quantize_resolution', None)
            auto_quantize = train_config.get('time_auto_quantize', False) # Default to False if not present
            data_sample_s = train_config.get('data_sample_size', 1000)
            min_valid_hist = train_config.get('min_valid_history', 1)

            base_dataset = TimeAwareJSONLDataset(
                data_path=data_path,
                time_normalization_method=time_norm_method,
                quantize_resolution=quantize_res,
                auto_quantize=auto_quantize,
                sample_size=data_sample_s
            )
            ...
            log_in_local_rank_0(f"Using TimeAwareWindowDataset with time normalization: {time_norm_method or 'None'}")
             ...
             seq_norm_method = train_config.get("sequence_normalization_method", 'zero') # Default from main.py
             dataset = TimeMoEDataset(
                 data_path,
                 normalization_method=seq_norm_method
             )
             log_in_local_rank_0('Processing dataset to fixed-size sub-sequences using original TimeMoEWindowDataset...')
             # Import original explicitly if there's a name clash
        
             train_ds = OriginalTimeMoEWindowDataset(
                 dataset,
                 context_length=context_length,
                 prediction_length=0, # For autoregressive
                 shuffle=False # OriginalTimeMoEWindowDataset doesn't take shuffle in __init__ based on provided snippets
             )
             # Original TimeMoEWindowDataset might have its own collate or rely on default.
             # If it produces dicts compatible with default_data_collator, 'None' is fine.
             current_data_collator = None # Or specific collator if OriginalTimeMoEWindowDataset needs one
             log_in_local_rank_0(f"Using original TimeMoEWindowDataset with sequence normalization: {seq_norm_method or 'None'}")
        ...
        if is_time_aware_dataset and data_path.endswith('.jsonl'):
            ...
            train_ds = window_dataset # Use this as the training dataset
             ...
             # Use the original TimeMoEWindowDataset for this case
             from time_moe.datasets.time_moe_window_dataset import TimeMoEWindowDataset as OriginalTimeMoEWindowDataset
             window_dataset = OriginalTimeMoEWindowDataset(
                 dataset,
                 context_length=max_length,
                 prediction_length=0,
                 shuffle=False

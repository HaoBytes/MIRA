...
class MIRAForPrediction(MIRAPreTrainedModel, MIRAGenerationMixin):
    ...
    def forward(
        ...
        if (
            ...
            hidden_states_for_head = self.ode_extrapolation_block(
                ...
                t_Nplus1=next_target_time_values.unsqueeze(-1)
            )
        else:
    ...
    #     """允许 time_values 和 next_target_time_values 参数"""
        
    #     extra_valid_params = {'time_values', 'next_target_time_values'}
    #     custom_kwargs = {k: model_kwargs.pop(k) for k in extra_valid_params if k in model_kwargs}
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        if past_key_values is not None:
            ...
            # 情况1: attention_mask 更长（前缀只在 cache 里）
            ...
            # 情况3: cache + 新 token 超过 max_cache_length
            if (
        ...
        # ---- 生成 Position IDs (用于标准 RoPE，如果不用 CT-RoPE 的话) ----
        ...
        if next_target_time_values is not None:
            ...
            if time_values is not None and self.use_terminal_ode:
                ...
                if time_step is None and time_values.shape[1] > 1:
                    ...
                    time_step = time_diffs.mean(dim=1, keepdim=True)  # [B, 1]
